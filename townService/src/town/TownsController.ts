...
export class TownsController extends Controller {
  ...
  /**
   * Creates a voting area in a given town
   * @param townID ID of the town in which to create the new voting area
   * @param sessionToken session token of the player making the request, must match the session token returned when the player joined the town
   * @param requestBody The new voting area to create
   */
  @Post('{townID}/votingArea')
  @Response<InvalidParametersError>(400, 'Invalid values specified')
  public async createVotingArea(
    @Path() townID: string,
    @Header('X-Session-Token') sessionToken: string,
    @Body() requestBody: Omit<VotingArea, 'type'>,
  ): Promise<void> {
    const town = this._townsStore.getTownByID(townID);
    if (!town?.getPlayerBySessionToken(sessionToken)) {
      throw new InvalidParametersError('Invalid values specified');
